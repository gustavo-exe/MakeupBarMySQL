...
CREATE TABLE IF NOT EXISTS `makeupbar`.`productogeneral` (
  `idCodigoDeBarra` INT NOT NULL DEFAULT 10010000000,
...
DELIMITER $$
USE `makeupbar`$$
CREATE DEFINER = CURRENT_USER TRIGGER `makeupbar`.`maquillaje_AFTER_INSERT` AFTER INSERT ON `maquillaje` FOR EACH ROW
BEGIN
...
USE `makeupbar`$$
CREATE DEFINER = CURRENT_USER TRIGGER `makeupbar`.`productogeneral_AFTER_INSERT` AFTER INSERT ON `productogeneral` FOR EACH ROW
BEGIN
